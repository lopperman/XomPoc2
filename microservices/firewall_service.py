...
def callback(ch, method, properties, body):
    ...
    print(f"Processing Firewall: {device_data['fqdn']}")

    # Simulate processing
    device_data['history'].append({'action': 'processed_by_firewall_service', 'timestamp': datetime.now().isoformat()})
...
channel.basic_consume(queue='firewall_queue', on_message_callback=callback, auto_ack=True)

print('Firewall Service is waiting for messages...')
