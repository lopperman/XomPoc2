...
def callback(ch, method, properties, body):
    ...
    print(f"Processing Switch: {device_data['fqdn']}")

    # Simulate processing
    device_data['history'].append({'action': 'processed_by_switch_service', 'timestamp': datetime.now().isoformat()})
...
channel.basic_consume(queue='switch_queue', on_message_callback=callback, auto_ack=True)

print('Switch Service is waiting for messages...')
